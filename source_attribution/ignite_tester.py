...
test_metrics = {
    # "loss": Loss(criterion),
    "precision": Precision(
        average=False, is_multilabel=True, output_transform=binary_transform
    ),
    "recall": Recall(
        average=False, is_multilabel=True, output_transform=binary_transform
    ),
...
print(
    f"""Test Results:
    \t Mean Precision :{torch.mean(prec):.3f}
    \t Mean Recall    :{torch.mean(reca):.3f}\n"""
)
print("By Label:")
print("\tLabel - Precision - Recall")
for l, p, r in zip(labels, prec, reca):
    print(f"\t{l} - {p:.3f} - {r:.3f}")
# print(
#     f"""Test Results:]
...
#     f"/data/boo/SOURCE_ATTRIBUTION/checkpoints/microresnet_{args.phoneme}_{timestamp}",
#     n_saved=2,
